...
public class PersyClient {
	 ...
	 * account) and contains custom logging settings.
	 *
	 ...
	 */
	public PersyClient(String credAccountId, String credAuthToken, String accountId, LogWriter writer) throws PersyException {
		this.credAccountId = credAccountId;
		this.credAuthToken = credAuthToken;
		this.accountId = accountId;

		this.accounts = new AccountRequester(credAccountId, credAuthToken, accountId, writer);
		this.recordings = new RecordingsRequester(credAccountId, credAuthToken, accountId, writer);
		this.calls = new CallsRequester(credAccountId, credAuthToken, accountId, writer);
		this.conferences = new ConferencesRequester(credAccountId, credAuthToken, accountId, writer);
		this.logs = new LogRequester(credAccountId, credAuthToken, accountId, writer);
		this.available = new AvailablePhoneNumberRequester(credAccountId, credAuthToken, accountId, writer);
		this.calling = new CallingNumberRequester(credAccountId, credAuthToken, accountId, writer);
		this.incoming = new IncomingPhoneNumberRequester(credAccountId, credAuthToken, accountId, writer);
		this.queues = new QueuesRequester(credAccountId, credAuthToken, accountId, writer);
		this.applications = new ApplicationsRequester(credAccountId, credAuthToken, accountId, writer);
	}
	...
	/**
	 * This constructor allows one to create a PersyClient that authenticates
	 * with one set of credentials and acts as that account and contains
	 ...
	 * @param credAuthToken The Auth Token to use in your credentials for the Persephony API. This should be the matching Auth Token to the credAccountId parameter.
	 * @param writer The log writer to use.
	 * @see #PersyClient(String, String, String, LogWriter)
	 */
	public PersyClient(String credAccountId, String credAuthToken, LogWriter writer) throws PersyException {
		this(credAccountId, credAuthToken, credAccountId, writer);
	}
